...
Default range is 2018-01-01 through today. After this finishes, check
/trips "Ожидают подтверждения" for anything that didn't import cleanly
(unmapped work type, unresolved boat color, etc.) — those need a manual
look, same as any other YCLIENTS import candidate.
"""
...
def main():
    ...
    parser.add_argument("--start", default="2018-01-01", help="YYYY-MM-DD, default 2018-01-01")
    ...
    if last_pending:
        print(
            f"В очереди «Ожидают подтверждения» на странице /trips сейчас "
            f"{last_pending} записей — разберите их вручную перед тем как "
            "считать бэкфилл завершённым."
        )
