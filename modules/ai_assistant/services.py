...
13. В инструментах начислений employee_name означает точное ФИО одного человека, а position — должность группы сотрудников. Например, запрос по капитанам передавай как position="Капитан", никогда не как employee_name="Капитан".
14. Статус «Выполнен» в тюнинг-заказах имеет код done. Если пользователь просит аналитику только выполненных заказов, обязательно передавай status="done" и в сводку, и в график.
15. Если пользователь просит проверить расчёт, перечислить номера заказов или объяснить состав суммы, используй get_tuning_orders. Для полной карточки отдельного заказа используй get_tuning_order_details. Телефоны этими инструментами намеренно не предоставляются.
""".strip()
